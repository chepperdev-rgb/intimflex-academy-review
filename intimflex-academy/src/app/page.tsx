...
const getBrowserLanguage = (): Lang => {
  const value = document.documentElement.dataset.academyLang;
  return value === "RU" || value === "ES" ? value : "EN";
};
const subscribeToLanguage = (notify: () => void) => {
  window.addEventListener(languageEvent, notify);
  return () => window.removeEventListener(languageEvent, notify);
};
...
const faqOverrides: Record<Lang, readonly (readonly [string, string])[]> = {
  EN: [
    ...
    ["How much does the training cost and how can I pay?", "The full cost of the training and certification is $2,499 USD.\n\nYou can pay the full amount upfront or reserve your place with a non-refundable $499 deposit. The deposit is included in the total cost. The remaining $2,000 must be paid by October 10, 2026."],
    ["How is the training delivered and how long does it last?", "The program is fully online. The training, practical assignments, feedback, final exam and retake are all conducted remotely. You can participate from any country.\n\nThe training lasts one month. After that, you have an additional two months for independent practice, preparation and taking the final exam."],
    ["Do I need to purchase the main AXS INTIMFLEX course separately?", "No. The complete core AXS INTIMFLEX method is already included in the trainer training program.\n\nYou study it from an instructor’s perspective: how to perform the exercises, explain them to clients, correct technique and structure sessions."],
    ["How do I receive the certificate, and can I retake the exam?", "Complete the training, fulfil the required assignments and successfully pass the final exam.\n\nYou will then receive a named AXS INTIMFLEX Academy Certified Trainer certificate with a unique number. It confirms that you have successfully completed our trainer training program.\n\nIf you do not pass the exam on your first attempt, you can retake it during the two-month assessment period after the training.\n\nPayment for the training alone does not guarantee that you will receive a certificate."],
    ["What happens after payment?", "After full payment or payment of the deposit, you will receive access to a private WhatsApp group with Ilona. It will contain information about the start of the training, preparation and next steps."],
    ["What will I be able to do after certification?", "You will be able to apply the AXS INTIMFLEX method in your independent work under your own name or brand: conduct personal and group sessions online and offline, set prices, create your schedule and grow your client base.\n\nA separate membership is available to promote sessions under the AXS INTIMFLEX brand and use the approved logo.\n\nThe certificate does not give you the right to conduct trainer certification programs or issue certificates on behalf of AXS INTIMFLEX."],
    ["Is membership required after certification?", "No. There are no mandatory subscriptions, annual fees or certificate renewal fees for teaching independently under your own name or brand.\n\nAXS INTIMFLEX Trainer Membership is optional after certification and costs $150 USD per month. The membership is paid separately and is not included in the training cost. There is no free period."],
    ["What does AXS INTIMFLEX Trainer Membership provide?", "The membership provides AXS INTIMFLEX Active Licensed Trainer status and the right to use the approved name and logo to promote your sessions under the membership terms.\n\nYou receive a personal profile in the official trainer directory with your city, country and contact details.\n\nOur team brings an international audience to the website from social media. Women can find you for sessions in their city or online and contact you directly. This gives you the opportunity to receive enquiries from prospective clients through the official website.\n\nThe membership also includes additional professional materials, method updates, a private trainer community and Academy support."],
    ["Does AXS INTIMFLEX take a percentage of my sessions?", "No. Royalties on personal and group sessions that you conduct independently are 0%.\n\nThis applies whether you work under your own name or brand, or under the AXS INTIMFLEX brand with an active membership. The membership has a fixed cost; no additional percentage of your session income is charged."],
    ["What happens if I end my membership?", "Your certificate of training remains valid. You can continue teaching independently under your own name or brand and state the qualification you received.\n\nYour Active Licensed Trainer status is removed and your profile is deleted from the official directory. Access to additional materials, updates, the private community and membership support ends.\n\nThe right to use the licensed logo and promote sessions under the AXS INTIMFLEX brand ends when the membership expires."],
  ],
  RU: [
    ["Когда начинается программа?", "Первый поток начинается 12 октября 2026 года."],
    ["Сколько мест доступно?", "В первом потоке только 10 мест. Это позволяет уделить внимание каждой участнице, проверять технику и давать персональную обратную связь."],
    ["Сколько стоит обучение и как оплатить?", "Полная стоимость обучения и сертификации составляет $2,499 USD.\n\nТы можешь оплатить всю сумму сразу или забронировать место невозвратным депозитом $499. Депозит входит в общую стоимость. Оставшиеся $2,000 необходимо оплатить до 10 октября 2026 года."],
    ["Как проходит обучение и сколько оно длится?", "Программа полностью онлайн. Обучение, практические задания, обратная связь, финальный экзамен и пересдача проходят дистанционно. Ты можешь участвовать из любой страны.\n\nОбучение длится один месяц. После него у тебя есть ещё два месяца на самостоятельную практику, подготовку и сдачу финального экзамена."],
    ["Нужно ли отдельно покупать основной курс AXS INTIMFLEX?", "Нет. Полная основная методика AXS INTIMFLEX уже входит в программу подготовки тренеров.\n\nТы изучаешь её с позиции преподавателя: как выполнять упражнения, объяснять их клиенткам, корректировать технику и выстраивать занятия."],
    ["Как получить сертификат и можно ли пересдать экзамен?", "Пройди обучение, выполни обязательные задания и успешно сдай финальный экзамен.\n\nПосле этого ты получишь именной сертификат AXS INTIMFLEX Academy Certified Trainer с уникальным номером. Он подтверждает успешное прохождение нашей программы подготовки тренеров.\n\nЕсли ты не сдашь экзамен с первой попытки, сможешь пересдать его в течение двухмесячного периода аттестации после обучения.\n\nОплата обучения сама по себе не гарантирует получение сертификата."],
    ["Что происходит после оплаты?", "После полной оплаты или внесения депозита ты получишь доступ к закрытой группе Telegram с Илоной. Там будет информация о начале обучения, подготовке и дальнейших действиях."],
    ["Что я смогу делать после сертификации?", "Ты сможешь применять методику AXS INTIMFLEX в самостоятельной работе под своим именем или собственным брендом: проводить персональные и групповые занятия онлайн и офлайн, устанавливать цены, формировать расписание и развивать клиентскую базу.\n\nДля продвижения занятий под брендом AXS INTIMFLEX и использования утверждённого логотипа предусмотрена отдельная подписка.\n\nСертификат не даёт права проводить программы сертификации тренеров или выдавать сертификаты от имени AXS INTIMFLEX."],
    ["Обязательна ли подписка после сертификации?", "Нет. Для самостоятельного преподавания под своим именем или собственным брендом обязательных подписок, ежегодных взносов и платы за продление сертификата нет.\n\nAXS INTIMFLEX Trainer Membership подключается добровольно после сертификации и стоит $150 USD в месяц. Подписка оплачивается отдельно и не входит в стоимость обучения. Бесплатный период не предусмотрен."],
    ["Что даёт подписка AXS INTIMFLEX Trainer Membership?", "Подписка предоставляет статус AXS INTIMFLEX Active Licensed Trainer и право использовать утверждённые название и логотип для продвижения своих занятий на условиях подписки.\n\nТы получаешь персональный профиль в официальном каталоге тренеров с городом, страной и контактами.\n\nНаша команда привлекает на сайт международную аудиторию из социальных сетей. Женщины могут найти тебя для занятий в своём городе или онлайн и связаться напрямую. Это даёт возможность получать обращения потенциальных клиенток через официальный сайт.\n\nТакже в подписку входят дополнительные профессиональные материалы, обновления методики, закрытое сообщество тренеров и поддержка Академии."],
    ["Берёт ли AXS INTIMFLEX процент с моих занятий?", "Нет. Роялти с персональных и групповых занятий, которые ты проводишь самостоятельно, составляют 0%.\n\nЭто действует как при работе под своим именем или собственным брендом, так и при работе под брендом AXS INTIMFLEX с действующей подпиской. Подписка имеет фиксированную стоимость, дополнительный процент с дохода от занятий не взимается."],
    ["Что будет, если я прекращу подписку?", "Твой сертификат о прохождении обучения сохраняется. Ты можешь продолжать самостоятельное преподавание под своим именем или собственным брендом и указывать полученную квалификацию.\n\nСтатус Active Licensed Trainer снимается, профиль удаляется из официального каталога. Доступ к дополнительным материалам, обновлениям, закрытому сообществу и поддержке по подписке прекращается.\n\nПраво использовать лицензируемый логотип и продвигать занятия под брендом AXS INTIMFLEX прекращается с окончанием действия подписки."],
  ],
  ES: [
    ...
    ["¿Cuánto cuesta la formación y cómo puedo pagar?", "El coste total de la formación y la certificación es de $2,499 USD.\n\nPuedes pagar el importe completo de una vez o reservar tu plaza con un depósito no reembolsable de $499. El depósito está incluido en el coste total. Los $2,000 restantes deben abonarse antes del 10 de octubre de 2026."],
    ["¿Cómo se imparte la formación y cuánto dura?", "El programa es completamente online. La formación, las tareas prácticas, el feedback, el examen final y la repetición del examen se realizan a distancia. Puedes participar desde cualquier país.\n\nLa formación dura un mes. Después tendrás otros dos meses para practicar de forma independiente, prepararte y realizar el examen final."],
    ["¿Necesito comprar por separado el curso principal de AXS INTIMFLEX?", "No. El método principal completo de AXS INTIMFLEX ya está incluido en el programa de formación de trainers.\n\nLo estudias desde la perspectiva de una instructora: cómo realizar los ejercicios, explicárselos a tus clientas, corregir la técnica y estructurar las sesiones."],
    ["¿Cómo obtengo el certificado y puedo repetir el examen?", "Completa la formación, realiza las tareas obligatorias y supera el examen final.\n\nDespués recibirás un certificado nominativo AXS INTIMFLEX Academy Certified Trainer con un número único. Este confirma que has completado con éxito nuestro programa de formación de trainers.\n\nSi no superas el examen en el primer intento, podrás repetirlo durante el periodo de evaluación de dos meses posterior a la formación.\n\nEl pago de la formación por sí solo no garantiza la obtención del certificado."],
    ["¿Qué ocurre después del pago?", "Después de realizar el pago completo o abonar el depósito, recibirás acceso a un grupo privado de WhatsApp con Ilona. Allí encontrarás información sobre el inicio de la formación, la preparación y los siguientes pasos."],
    ["¿Qué podré hacer después de certificarme?", "Podrás aplicar el método AXS INTIMFLEX en tu trabajo independiente bajo tu propio nombre o marca: impartir sesiones individuales y grupales online y presenciales, fijar precios, crear tu horario y desarrollar tu base de clientas.\n\nExiste una membresía independiente para promocionar sesiones bajo la marca AXS INTIMFLEX y utilizar el logotipo aprobado.\n\nEl certificado no te da derecho a impartir programas de certificación de trainers ni a emitir certificados en nombre de AXS INTIMFLEX."],
    ["¿Es obligatoria la membresía después de la certificación?", "No. Para enseñar de forma independiente bajo tu propio nombre o marca, no hay suscripciones obligatorias, cuotas anuales ni cargos por renovar el certificado.\n\nAXS INTIMFLEX Trainer Membership se activa de forma voluntaria después de la certificación y cuesta $150 USD al mes. La membresía se paga por separado y no está incluida en el coste de la formación. No existe un periodo gratuito."],
    ["¿Qué ofrece AXS INTIMFLEX Trainer Membership?", "La membresía te otorga el estatus AXS INTIMFLEX Active Licensed Trainer y el derecho a utilizar el nombre y el logotipo aprobados para promocionar tus sesiones conforme a las condiciones de la membresía.\n\nRecibes un perfil personal en el directorio oficial de trainers con tu ciudad, país y datos de contacto.\n\nNuestro equipo atrae al sitio web una audiencia internacional desde las redes sociales. Las mujeres pueden encontrarte para realizar sesiones en su ciudad u online y ponerse en contacto contigo directamente. Esto te da la oportunidad de recibir consultas de clientas potenciales a través del sitio web oficial.\n\nLa membresía también incluye materiales profesionales adicionales, actualizaciones del método, una comunidad privada de trainers y soporte de la Academia."],
    ["¿AXS INTIMFLEX cobra un porcentaje de mis sesiones?", "No. Los royalties de las sesiones individuales y grupales que impartes de forma independiente son del 0%.\n\nEsto se aplica tanto si trabajas bajo tu propio nombre o marca como si lo haces bajo la marca AXS INTIMFLEX con una membresía activa. La membresía tiene un coste fijo; no se cobra ningún porcentaje adicional de los ingresos de tus sesiones."],
    ["¿Qué ocurre si cancelo mi membresía?", "Tu certificado de formación se mantiene. Puedes seguir enseñando de forma independiente bajo tu propio nombre o marca e indicar la cualificación obtenida.\n\nSe retira tu estatus Active Licensed Trainer y tu perfil se elimina del directorio oficial. Finaliza el acceso a los materiales adicionales, las actualizaciones, la comunidad privada y el soporte de la membresía.\n\nEl derecho a utilizar el logotipo licenciado y promocionar sesiones bajo la marca AXS INTIMFLEX termina cuando finaliza la membresía."],
  ],
} as const;
...
export default function Home() {
  const lang = useSyncExternalStore<Lang>(subscribeToLanguage, getBrowserLanguage, () => "EN");
  ...
  const selectLanguage = (next: Lang) => {
    ...
    document.documentElement.setAttribute("data-academy-lang", next);
    window.dispatchEvent(new Event(languageEvent));
  };
  const t = normalizeCopy(content[lang], lang) as typeof content[typeof lang];
  const onlineCourse = lang === "RU" ? "ОНЛАЙН-КУРС" : lang === "ES" ? "CURSO ONLINE" : "ONLINE COURSE";
  ...
  const termsCopy = lang === "RU"
    ? { label: "Условия и юридическая информация", file: "/AXS_INTIMFLEX_Academy_Terms_and_Legal_Information_RU_Russian.pdf" }
    : lang === "ES"
      ? { label: "Términos y condiciones", file: "/AXS_INTIMFLEX_Academy_Terms_and_Legal_Information_ES_Spanish.pdf" }
      : { label: "Terms & Conditions", file: "/AXS_INTIMFLEX_Academy_Terms_and_Legal_Information_EN_English.pdf" };
  ...
  return <main id="top">
    ...
    <section className="hero" aria-labelledby="hero-title"><video className="hero-video" autoPlay muted loop playsInline poster={asset("/img/hero-m.jpg")} aria-hidden="true"><source src={asset("/video/hero.mp4")} type="video/mp4" /></video><div className="hero-overlay" aria-hidden="true" /><div className="shell hero-grid"><div className="hero-copy"><div className="hero-eyebrow-line"><p className="eyebrow pink">{t.heroEyebrow}</p><span className="online-course-indicator"><i aria-hidden="true" /><b aria-hidden="true">|</b><span>{onlineCourse}</span><b aria-hidden="true">|</b><svg viewBox="0 0 24 24" role="img" aria-label={lang === "RU" ? "Глобальный онлайн-курс" : lang === "ES" ? "Curso online global" : "Global online course"}><circle cx="12" cy="12" r="9" /><path d="M3 12h18M12 3c2.35 2.46 3.55 5.46 3.55 9S14.35 18.54 12 21M12 3C9.65 5.46 8.45 8.46 8.45 12S9.65 18.54 12 21" /></svg></span></div><h1 id="hero-title">{t.heroTitle}</h1><p className="hero-lede">{t.heroLede}</p><div className="hero-meta"><span>{t.cohort}</span><span>{t.date}</span><span>{t.places}</span></div><div className="hero-actions"><a className="button button-primary" href="#investment">{t.enrollNow}</a><a className="button button-ghost" href="#investment">{t.reserveSpot}</a></div></div><IlonaCard lang={lang} eyebrow={t.heroEyebrow} cohort={t.cohort} spotsLabel={spotsLabel} /></div><div className="shell hero-facts">{t.facts.map((fact, i) => <span key={fact} className={i === 3 ? "last-fact" : ""}>{fact}</span>).flatMap((node, i, all) => i < all.length - 1 ? [node, <i key={`arrow-${i}`}>→</i>] : [node])}</div></section>
