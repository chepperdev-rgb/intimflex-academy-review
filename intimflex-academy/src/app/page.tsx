...
export default function Home() {
  ...
  return <main id="top">
    ...
    <section className="final-cta"><div className="final-cta-visual" aria-hidden="true"><Image src={asset("/img/academy-final-training.jpg")} alt="" fill sizes="100vw" /></div><div className="final-cta-tint" aria-hidden="true" /><div className="shell final-cta-content"><p className="eyebrow">{t.finalEyebrow}</p><h2>{t.finalTitle}</h2><p>{t.finalCopy}</p><div className="final-actions"><button className="button button-dark" type="button" onClick={() => setPaymentModal("full")}>{t.enrollNow}</button><button className="button button-dark-outline" type="button" onClick={() => setPaymentModal("deposit")}>{t.reserveSpot}</button></div></div></section>
    <footer className="footer"><div className="shell footer-top"><Logo /><div className="languages" role="group" aria-label={t.language}>{(["EN", "RU", "ES"] as Lang[]).map((item) => <button className={lang === item ? "selected" : ""} type="button" aria-pressed={lang === item} onClick={() => { setLang(item); setCertOpen(null); }} key={item}>{item}</button>)}</div><a href="mailto:axs.intimflex@gmail.com">axs.intimflex@gmail.com</a></div><div className="shell footer-bottom"><span>{t.footer}</span><a className="footer-terms" href={asset(termsCopy.file)} target="_blank" rel="noopener noreferrer">{termsCopy.label}</a><span>© {new Date().getFullYear()} AXS INTIMFLEX</span></div></footer>
